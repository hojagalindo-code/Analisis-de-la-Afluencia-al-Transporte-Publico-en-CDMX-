...
# Carga y tratamiento del dataset
 # ------------ Carga -----------------
data_tl=pd.read_csv('afluencia_desglosada_tl_05_2025.csv')

 # Normaliza el nombre de las líneas en la columna linea
#data_tl['linea'] = data_tl['linea'].str.lower().str.replace('línea', 'linea', regex=False).str.strip()

 # Verifica de que la columna 'fecha' esté en formato de fecha
data_tl['fecha'] = pd.to_datetime(data_tl['fecha'])

 # Ahora, crea la nueva columna 'dia_semana' extrayendo el nombre del día
data_tl['dia_semana'] = data_tl['fecha'].dt.day_name()
...
c1=data_tl['afluencia'].quantile(0.25)
c3=data_tl['afluencia'].quantile(0.75)
#----------------------------------------------
...
data_tl['categoria_afluencia'] = data_tl['afluencia'].apply(clasificador_afluencia)
...
X = data_tl[['anio','mes','dia_semana']]
y = data_tl['categoria_afluencia']

# --- Aplica One-Hot Encoding a la variable 'mes' que está en X. ---
X_preprocesado = pd.get_dummies(X, columns=['mes','dia_semana'], drop_first=True)
...
X_preprocesado = pd.get_dummies(X, columns=['mes'], drop_first=True)
...
print(data_tl['categoria_afluencia'].value_counts())
...
lineas_transporte = sorted(data_tl['linea'].unique())
...
plt.savefig('matriz_de_confusion_tl.png', dpi=300, bbox_inches='tight')
plt.show()
