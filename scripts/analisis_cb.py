...
# Carga y tratamiento del dataset
 # ------------ Carga -----------------
data_cb=pd.read_csv('afluencia_desglosada_cb_05_2025.csv')

 # Normaliza el nombre de las líneas en la columna linea
data_cb['linea'] = data_cb['linea'].str.lower().str.replace('línea', 'linea', regex=False).str.strip()

 # Verifica de que la columna 'fecha' esté en formato de fecha
data_cb['fecha'] = pd.to_datetime(data_cb['fecha'])

 # Ahora, crea la nueva columna 'dia_semana' extrayendo el nombre del día
data_cb['dia_semana'] = data_cb['fecha'].dt.day_name()
...
c1=data_cb['afluencia'].quantile(0.25)
c3=data_cb['afluencia'].quantile(0.75)
#----------------------------------------------
...
data_cb['categoria_afluencia'] = data_cb['afluencia'].apply(clasificador_afluencia)
...
X = data_cb[['anio','mes','linea','dia_semana']]
y = data_cb['categoria_afluencia']
...
print(data_cb['categoria_afluencia'].value_counts())
...
lineas_transporte = sorted(data_cb['linea'].unique())
...
plt.savefig('matriz_de_confusion_cb.png', dpi=300, bbox_inches='tight')
plt.show()
