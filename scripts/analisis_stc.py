...
# Carga y tratamiento del dataset
 # ------------ Carga -----------------
data_stc=pd.read_csv('afluenciastc_desglosado_05_2025.csv')

 # Normaliza el nombre de las líneas en la columna linea



 # Verifica que la columna 'fecha' esté en formato de fecha
data_stc['fecha'] = pd.to_datetime(data_stc['fecha'])

 # Ahora, crea la nueva columna 'dia_semana' extrayendo el nombre del día
data_stc['dia_semana'] = data_stc['fecha'].dt.day_name()
...
c1=data_stc['afluencia'].quantile(0.25)
c3=data_stc['afluencia'].quantile(0.75)
#----------------------------------------------
...
data_stc['categoria_afluencia'] = data_stc['afluencia'].apply(clasificador_afluencia)
...
X = data_stc[['anio','mes','linea','dia_semana']]
y = data_stc['categoria_afluencia']
...
# --- Evaluar el modelo y hacer una predicción ---
...
# --- Aplica SMOTE y entrena el modelo FINAL ---
...
print(data_stc['categoria_afluencia'].value_counts())
...
lineas_transporte = sorted(data_stc['linea'].unique())
