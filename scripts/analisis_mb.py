...
# Carga y tratamiento del dataset
 # ------------ Carga -----------------
data_mb=pd.read_csv('afluenciamb_desglosado_05_2025.csv')

 # Normaliza el nombre de las líneas en la columna linea
data_mb['linea'] = data_mb['linea'].str.lower().str.replace('línea', 'linea', regex=False).str.strip()

 # Verifica que la columna 'fecha' esté en formato de fecha
data_mb['fecha'] = pd.to_datetime(data_mb['fecha'])

 # Ahora, crea la nueva columna 'dia_semana' extrayendo el nombre del día
data_mb['dia_semana'] = data_mb['fecha'].dt.day_name()
...
c1=data_mb['afluencia'].quantile(0.25)
c3=data_mb['afluencia'].quantile(0.75)
#----------------------------------------------
...
data_mb['categoria_afluencia'] = data_mb['afluencia'].apply(clasificador_afluencia)
...
X = data_mb[['anio','mes','linea','dia_semana']]
y = data_mb['categoria_afluencia']
...
print(data_mb['categoria_afluencia'].value_counts())
...
lineas_transporte = sorted(data_mb['linea'].unique())
...
plt.savefig('matriz_de_confusion_mb.png', dpi=300, bbox_inches='tight')
plt.show()
